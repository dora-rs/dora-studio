...
/// A request sent to the background SigNoz runtime thread.
///
/// These requests are dispatched over an unbounded channel to be handled
/// asynchronously by the telemetry backend.
#[derive(Debug, Clone)]
pub enum SignozRequest {
    /// Request to perform a health check to verify connectivity.
    HealthCheck,
    /// Request to query traces using the given parameters.
    QueryTraces(TraceQuery),
}

/// A response generated by the background SigNoz runtime thread.
///
/// These responses are collected into a pending queue and polled by
/// the main UI thread during its event loop.
#[derive(Debug, Clone)]
pub enum SignozResponse {
    /// Indicates the health check was successful.
    HealthOk,
    /// Indicates the health check failed with the provided error message.
    HealthError(String),
    /// Contains the successful result of a trace query.
    Traces(Vec<Span>),
    /// Indicates a trace query failed with the provided error message.
    TracesError(String),
}

/// The current connection status of the telemetry bridge.
///
/// This status is continuously updated based on the result of background
/// health checks.
#[derive(Debug, Clone, Copy, PartialEq, Default)]
pub enum ConnectionStatus {
    /// Status is unknown (e.g. before the first health check has completed).
    #[default]
    Unknown,
    ...
    Disconnected,
    /// Connection or authentication error occurred.
    Error,
}
